...
基于115网盘开发者API文档实现了以下功能：

#### 核心API方法

1. **get_file_info** - 获取文件(夹)详情
   - 支持通过`file_id`或`path`获取文件详情
   - 返回文件大小、数量、视频时长等信息
   - 支持`/`和`>`两种路径分隔符
...
async def _get_cloud_realtime_changes(self) -> Dict[str, List[Dict[str, Any]]]:
    """获取网盘实时文件变更"""
    # 获取115网盘API客户端
    cloud_115_api = Cloud115API(access_token=self.access_token)
    
    # 获取上次同步时间
    last_sync_time = await self._get_last_sync_time()
    
    # 获取文件变更
    changes = await cloud_115_api.get_file_changes(
...
async def _scan_cloud_changes(self, last_sync_time: datetime) -> List[Dict[str, Any]]:
    ...
    # 根据时间范围搜索文件
    files = await cloud_115_api.search_files_by_time_range(
